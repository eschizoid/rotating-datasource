 ...
 *
 * <pre>{@code
 * final var rotatingCf = RotatingConnectionFactory.builder()
...
public final class RotatingConnectionFactory implements ConnectionFactory {
  ...
  private static final Logger LOGGER = getLogger(RotatingConnectionFactory.class.getName());
  ...
  private void checkAndRefresh() {
    try {
      ...
      LOGGER.log(Logger.Level.WARNING, "Failed to check secret version", e);
    }
  }
  ...
  private Mono<Void> checkLatestVersionAndRefreshIfNeeded() {
    return Mono.fromCallable(() -> SecretHelper.getSecretVersion(secretId))
        ...
        .onErrorResume(
            e -> {
              LOGGER.log(Logger.Level.WARNING, "Version check failed", e);
              return Mono.empty();
            });
  }
