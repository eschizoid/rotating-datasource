...
public final class Retry {
  ...
  private static final Logger LOGGER = getLogger(Retry.class.getName());
   ...
   *
   * <pre>{@code
   * final var readResult = Retry.authRetry(
   *     () -> readOnlyRepository.findAll(),
   *     readOnlyDs::reset,
   *     AuthErrorDetector.defaultDetector()
   * );
   *
   * final var writeResult = Retry.authRetry(
  ...
  public interface AuthErrorDetector {
     ...
     *
     * <pre>{@code
     * final var pgDetector = AuthErrorDetector.custom(e -> {
     *     final var state = e.getSQLState();
     ...
     * final var combined = AuthErrorDetector.defaultDetector()
   ...
   * final var fixed = Policy.fixed(3, 500L);
   *
   ...
   * final var custom = new Policy(10, 100L, 10_000L, 3.0, true);
   * }</pre>
   *
  ...
  public record Policy(
     ...
     * final var policy = Policy.fixed(5, 500L);
     *
