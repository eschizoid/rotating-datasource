...
import com.example.smrotator.core.Retry.SqlExceptionSupplier;
...
public record DbClient(RotatingDataSource rotatingDataSource) {
  ...
  public <T> T executeWithRetry(final DbOperation<T> operation) throws SQLException {
    ...
    return onException(supplier, shouldRetry, rotatingDataSource::reset, 2, 0L);
  }
  ...
  private <T> T run(final DbOperation<T> operation) throws SQLException {
    try (final var conn = rotatingDataSource.getDataSource().getConnection()) {
      return operation.execute(conn);
    }
  }

  /**
   * Functional database operation executed against an open {@link Connection}.
   *
   * @param <T> result type
   */
  @FunctionalInterface
