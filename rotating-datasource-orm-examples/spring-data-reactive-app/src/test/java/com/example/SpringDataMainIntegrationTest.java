...
public class SpringDataMainIntegrationTest {
  ...
  void setUp() {
    ...
    this.db = ctx.getBean(DatabaseClient.class);
    db.sql(
            """
        ...
        .block();
    for (int i = 0; i < 100; i++) {
      insertUser("user%d".formatted(i), "user%d@example.com".formatted(i));
    }
  }
  ...
  private String selectNow() {
    return db.sql("SELECT NOW()").map((row, md) -> String.valueOf(row.get(0))).one().block();
  }

  private void insertUser(final String username, final String email) {
    db.sql("INSERT INTO test_users (username, email) VALUES ($1, $2)")
        .bind(0, username)
        .bind(1, email)
        .fetch()
        .rowsUpdated()
  ...
  private boolean selectUserByIdExists(final long id) {
    final var list =
        db.sql("SELECT username FROM test_users WHERE id = $1")
            .bind(0, id)
            .map((row, md) -> row.get(0, String.class))
            .all()
            .collectList()
            .block();
  ...
  private int updateEmailForId(final long id, final String email) {
    final Number n =
        db.sql("UPDATE test_users SET email = $1 WHERE id = $2")
            .bind(0, email)
            .bind(1, id)
            .fetch()
            .rowsUpdated()
  ...
  private int insertTempAndDeleteOne(final String prefix, final String email) {
    final Number ins =
        db.sql("INSERT INTO test_users (username, email) VALUES ($1, $2)")
            .bind(0, prefix + System.nanoTime())
            .bind(1, email)
            .fetch()
            .rowsUpdated()
            .map(it -> (Number) it)
    ...
    final Number del =
        db.sql(
                "DELETE FROM test_users WHERE id IN (SELECT id FROM test_users WHERE username LIKE $1 || '%' ORDER BY id LIMIT 1)")
            .bind(0, prefix)
            .fetch()
            .rowsUpdated()
            .map(it -> (Number) it)
            .block();
    return (ins == null ? 0 : ins.intValue()) + (del == null ? 0 : del.intValue());
  }

  private int countUsers() {
    final Number n =
        db.sql("SELECT COUNT(*) FROM test_users")
            .map((row, md) -> (Number) row.get(0))
            .one()
            .block();
    return n == null ? 0 : n.intValue();
  }
        ...
        () -> {
          ...
          try {
            ...
            LOGGER.log(
                ERROR,
                String.format(
                    "[%s] Password rotation failed: %s%n", Instant.now(), exception.getMessage()));
            ...
            try {
              while (!stopFlag.get()) {
                try {
                  switch (workloadType) {
                    case 0 -> {
                      final var exists = selectUserByIdExists((workerId % 100) + 1);
                      if (exists) successfulOps.incrementAndGet();
                    }
                    case 1 -> {
                      final var id = (long) ((workerId % 100) + 1);
                      final var updated =
                          updateEmailForId(id, "updated" + workerId + "@example.com");
                      if (updated >= 0) successfulOps.incrementAndGet();
                    }
                    case 2 -> {
                      final var done =
                          insertTempAndDeleteOne("temp_", "temp" + workerId + "@example.com");
                      if (done >= 0) successfulOps.incrementAndGet();
                    }
                  }
      ...
      errors.stream()
          ...
          .forEach(
              e ->
                  LOGGER.log(
                      ERROR,
                      String.format("  - %s: %s%n", e.getClass().getSimpleName(), e.getMessage())));
    }

    final var count = countUsers();
